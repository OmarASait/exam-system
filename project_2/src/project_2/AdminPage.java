...
public class AdminPage extends JFrame {
    ...
    private void saveAndReset() {
        ...
        if (!teacherUsername.isEmpty() || !teacherPassword.isEmpty()) {
            saveData("teachers", teacherUsername, teacherPassword);  // تمرير اسم المستخدم وكلمة المرور فقط
            isTeacherSaved = true;
        }

        if (!studentUsername.isEmpty() || !studentPassword.isEmpty()) {
            saveData("student1", studentUsername, studentPassword);  // تمرير اسم المستخدم وكلمة المرور فقط
            isStudentSaved = true;
        }
